...
|---|---|---|---|---|
| **P0** | 闭环学习 | 反思记忆闭环（强化） | 🟡 有骨架 | M |
| **P0** | 决策质量 | 结构化输出（Trader/PM） | 🔴 缺 | S |
| **P0** | 可观测 | 成本/用量遥测面板 | 🟡 token 已采集 | S |
| **P1** | 工具生态 | MCP 化 23 个工具 | 🟢 `get_all_tools()` 统一 | M |
| **P1** | 韧性 | Checkpoint/Resume | 🟡 有 SQLite+StateMachine | M |
| **P1** | 抗幻觉 | Grounded Analyst（锚定价格） | 🔴 缺 | S |
| **P2** | 量化能力 | Alpha Zoo（因子库） | 🔴 backtest/ 仅 stub | L |
| **P2** | 量化能力 | 回测引擎套件（MC/WF/Bootstrap） | 🔴 同上 | L |
| **P2** | 差异化 | Shadow Account（行为诊断） | 🔴 缺 | L |
| **P3** | 研究脊梁 | Hypothesis Registry + Research Goal | 🔴 缺 | M |
| **P3** | 记忆/技能 | 跨会话记忆升级 + 自进化技能 | 🟡 有 PersistentMemory | M |
| **P3** | 产出 | 策略导出（NL→Pine/MQL5） | 🔴 缺 | S |
| **P3** | 编排 | Swarm 预设（投资委员会等） | 🟡 我们是固定管线 | M |
| **P4** | 广度 | OKX 实盘 + 更多 broker | 🔴 OKX 仅数据 | L |
| **P4** | 广度 | 多市场（A股/美股/外汇） | 🔴 crypto-only | XL |

> 🟢 易 / 🟡 有骨架需扩展 / 🔴 缺失需新建。工作量：S≈1–3 天，M≈1 周，L≈2–4 周，XL≈多月。
...
### P4.2 多市场（A股/美股/外汇/期货）
**借鉴**：TradingAgents 用 Yahoo ticker 覆盖全球；HKUDS 7 个市场引擎。

**缺口**：大数据源扩展（tushare/yfinance/akshare），且与现有 crypto 主线定位有取舍。**建议仅在确认要扩业务边界时做。**
