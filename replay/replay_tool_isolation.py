"""Replay tool isolation — 消除 look-ahead 污染（SWDA 2026-08-09 P0 Fix ①）。

問題：replay_leg_a.py 用完整 13-agent TradingCoordinator，agents 的 tools
（get_current_price / get_funding_rate / get_order_book ...）全部打 live API，
決策混入 2026-08-09 當下市場數據，而非 replay bar 的歷史數據 → 回測不可信。
...
def install_replay_tool_isolation(storage, interval: str = "30m") -> None:
    ...
    async def _replay_get_current_price(symbol: str, storage_: Optional = None) -> dict:
    ...
    async def _replay_get_kline_data(symbol: str, interval_: str = "30m", limit: int = 100) -> dict:
        klines = await storage.query_klines(
    ...
    mdt.get_kline_data = _replay_get_kline_data
    ...
    tt.get_kline_data = _replay_get_kline_data
